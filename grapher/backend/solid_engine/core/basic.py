import numpy as np
import contextlib
from grapher.backend.utils import as_array
...
class FuncNode(object):
    ...
    def __call__(self, is_backward: bool=False):
        if not is_backward:
            ...
            for node in self.connected_nodes[0]:
                self.forward_args.append(as_array(node.forward(*node.forward_args)))
        else:
            gy = as_array(self.gy)
    ...
    def connect(self, *nodes):
        ...
        generations = []
        for node in nodes: node.connected_nodes[1].append(self); generations.append(node.generation)
        self.generation = max(self.generation, max(generations)) + 1
...
class Number(FuncNode):
    ...
    def forward(self, *args):
        self.forward_args.append(self.data)
        return self.data
...
class Graph(object):
    ...
    def backward(self, base_grad: int | float=1):
        ...
        for node in nodes: node(is_backward=True) # ; print(node)
    ...
    def making_structure(self):
        ...
        self.nodes.sort(key=lambda x: x.generation)
